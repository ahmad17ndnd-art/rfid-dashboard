...
document.getElementById("btn-register").addEventListener("click", async () => {
  ...
  if (password.length < 6) {
    showLoginError(t("err_password_short"));
    return;
  }
  if (password !== confirmPassword) {
    showLoginError(t("err_password_mismatch"));
    return;
  }
  ...
  try {
    ...
    showLoginError(e.message === "Email already registered" ? t("err_email_exists") : e.message);
  }
});
