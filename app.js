...
function showLoginStep(step) {
  ["step-login", "step-set-password", "step-forgot", "step-reset", "step-register"].forEach(id => {
...
document.getElementById("btn-show-register").addEventListener("click", () => showLoginStep("step-register"));
...
document.getElementById("btn-back-login-3").addEventListener("click", () => showLoginStep("step-login"));
...
async function loadSettings() {
  try {
    const key = await api("/device/my-key");
    document.getElementById("device-key-value").value = key.device_api_key || "";
  } catch (e) { console.error(e); }
  try {
...
document.getElementById("btn-copy-device-key").addEventListener("click", async () => {
  const val = document.getElementById("device-key-value").value;
  if (!val) return;
  try {
    await navigator.clipboard.writeText(val);
    const btn = document.getElementById("btn-copy-device-key");
    const original = btn.textContent;
    btn.textContent = t("key_copied");
    setTimeout(() => { btn.textContent = original; }, 1500);
  } catch (e) { console.error(e); }
});

document.getElementById("btn-regenerate-device-key").addEventListener("click", async () => {
  if (!confirm(t("regenerate_key"))) return;
  try {
    const res = await api("/device/regenerate-key", { method: "POST" });
    document.getElementById("device-key-value").value = res.device_api_key;
  } catch (e) { alert(e.message); }
});
